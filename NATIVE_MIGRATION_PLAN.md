...
| Stablecoin and multi-channel support | Metadata prerequisite complete; financial migration pending | Ferret bundles the reviewed Konduit built-in catalog plus deterministic Koios presentation metadata/logos and verifies them offline in Android checks. It does not yet model asset-tagged amounts, fund native-asset channel outputs, persist multiple channels per wallet, or let the payer select an ADA, USDM, USDCx, or other supported channel. |
